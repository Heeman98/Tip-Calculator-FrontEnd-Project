...
let totalPayablePerPerson;

//used to show the value in Indian currency . 
...
function initializeTipButtons() {
  ...
  for (i = 0; i < tipButtons.length - 1; i++) {
    ...
    tipButtons[i].addEventListener("click", function () { customTip.value = ""; })
...
function calculate() {
  if (peopleTotalValue != 0 && tipPercentValue != 0) {
    ...
    totalPayablePerPerson = totalPerPersonValue + tipPerPersonValue;
    totalPerPerson.textContent = `${formatter.format(totalPayablePerPerson)}`;
  }
